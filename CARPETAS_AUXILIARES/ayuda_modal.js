...
if(!v||!f){v=a.closest('header,body')?.querySelector('.ventana');f=v?.querySelector('iframe')}if(!v||!f)return;let c=v.querySelector('.contenidoVentana')||v,urls=vistaDefault(a);
let pdf=resuelve(urls.pdf),docx=resuelve(urls.docx),ok=await esPdf(pdf);
if(!ok){f.src='';f.setAttribute('srcdoc','<div style="font:14px system-ui;padding:16px"><h3>Ayuda no disponible</h3><p>No se ha encontrado un PDF válido en <code>'+normalizaUrl(pdf)+'</code>.</p><p>Si existe una regla que redirige 404 a <code>index.html</code>, Cloudflare puede estar devolviendo HTML en vez del PDF.</p><p><a href="'+normalizaUrl(pdf)+'" target="_blank" rel="noopener">Abrir el recurso en una pestaña nueva</a></p></div>')}
else{f.removeAttribute('srcdoc');f.src=pdf}
