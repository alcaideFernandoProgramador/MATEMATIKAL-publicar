...
if(!caja1||!caja2)return;caja2.style.display="flex";caja2.style.flexDirection="column";caja2.style.overflow="hidden";
...
caja21.style.cssText="display:flex;flex-direction:column;gap:8px;flex:1 1 auto;min-height:0;overflow-y:auto;overflow-x:hidden;";
...
caja21Titulo.textContent="TRABAJO DEL USUARIO";let h=Math.max(240,Math.min(560,Math.floor(window.innerHeight*0.45)));caja21.style.setProperty("max-height",h+"px","important");caja21.style.setProperty("overflow-y","auto","important");caja21.style.setProperty("overflow-x","hidden","important");if(!caja21.__wheelLock){caja21.__wheelLock=true;caja21.addEventListener("wheel",e=>{if(caja21.scrollHeight<=caja21.clientHeight)return;e.preventDefault();caja21.scrollTop+=e.deltaY;},{passive:false});}
...
let t1=document.createElement("div");t1.textContent="rango(A) = "+r;t1.style.fontWeight="bold";t1.style.fontSize="17px";cont.appendChild(t1);
...
function mostrarMensajeRangoFinal(orden,menor,porDimension=false,porNulos=false){
ocultarMenorActual();
function construirEn(dest){
if(dest===cajaFormulario)dest.innerHTML="";
let cont=document.createElement("div");
...
let tit=document.createElement("div");tit.textContent="RESULTADO";
tit.style.cssText="font-weight:bold;font-size:16px;text-align:center;margin-bottom:6px;";cont.appendChild(tit);
let t1=document.createElement("div");t1.textContent="rango(A) = "+orden+".";t1.style.fontWeight="bold";t1.style.fontSize="17px";cont.appendChild(t1);
let t2=document.createElement("div");
t2.textContent=porDimension?"No hay ningún menor de orden "+(orden+1)+" no nulo por falta de filas o columnas.":(porNulos?"No hay ningún menor de orden "+(orden+1)+" no nulo porque todos son nulos.":"No hay ningún menor de orden "+(orden+1)+" no nulo.");
cont.appendChild(t2);
if(menor){
...
let filasAct=p2.filas.slice(),colsAct=p2.cols.slice(),razonParada=null;while(filasAct.length<rangoFinal){
let s=ampliarMenorActual(matriz,filasAct,colsAct);
if(s.fallo){razonParada=s.razonFallo;if(razonParada&&razonParada.tipo==="nulos"&&s.pasos&&s.pasos.length)pintarMenoresNulos(s.pasos,"Menor nulo de orden "+(filasAct.length+1)+" construido a partir del menor actual");break;}
...
let porDim=filasAct.length===matriz.length||colsAct.length===matriz[0].length,porNulos=razonParada&&razonParada.tipo==="nulos";mostrarSolucionFinalMenor(menorActual.menor);mostrarMensajeRangoFinal(filasAct.length,menorActual.menor,porDim,porNulos);desmarcarOpcionActual();return;
}
