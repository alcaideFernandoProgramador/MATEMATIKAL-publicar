...
=======
function _toggleMatrizSetup(){try{let panel=document.getElementById("panelMatriz"),btn=document.getElementById("btnToggleMatriz"),tit=document.getElementById("titulo3");if(!panel||!btn||!tit)return;let set=(on)=>{panel.classList.toggle("oculto",!on);btn.setAttribute("aria-expanded",on?"true":"false");btn.textContent=on?"Ocultar matriz":"Mostrar matriz";};btn.addEventListener("click",()=>set(panel.classList.contains("oculto")));let auto=()=>{let t=(tit.textContent||"").toLowerCase();if(t.includes("escalonada"))set(false);};auto();if(!window.__obsTit3){window.__obsTit3=new MutationObserver(auto);window.__obsTit3.observe(tit,{childList:true,subtree:true});}}catch(e){}}
>>>>>>> 17a26830cbecb34b461c8fc5e1f425de0f72877e
