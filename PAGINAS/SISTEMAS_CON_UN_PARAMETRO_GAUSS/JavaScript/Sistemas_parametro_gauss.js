...
let caja12=document.createElement("div");caja1.appendChild(caja12);caja12.id="caja12";caja12.style.height="94%";caja12.style.width="40%";caja12.style.display="block";caja12.style.padding="5px";caja12.style.marginRight="3px";caja12.style.marginTop="2px";caja12.style.alignItems="center";
...
function setupToggleTrabajos(){try{let btn=document.getElementById("btnToggleTrabajos"),wrap=document.getElementById("contenedorCaja3"),c3=document.getElementById("caja3");if(!btn||!wrap||!c3)return;let set=(on)=>{document.body.classList.toggle("trabajosOcultos",!on);btn.setAttribute("aria-expanded",on?"true":"false");btn.textContent=on?"Ocultar trabajos":"Mostrar trabajos";if(on)c3.scrollTop=c3.scrollHeight;};btn.addEventListener("click",()=>set(document.body.classList.contains("trabajosOcultos")));let sync=()=>{let esc=document.body.classList.contains("escalonada");btn.style.display=esc?"inline-flex":"none";if(!esc){document.body.classList.remove("trabajosOcultos");btn.setAttribute("aria-expanded","true");btn.textContent="Ocultar trabajos";}};sync();if(!window.__obsBodyCls){window.__obsBodyCls=new MutationObserver(sync);window.__obsBodyCls.observe(document.body,{attributes:true,attributeFilter:["class"]});}}catch(e){}}
