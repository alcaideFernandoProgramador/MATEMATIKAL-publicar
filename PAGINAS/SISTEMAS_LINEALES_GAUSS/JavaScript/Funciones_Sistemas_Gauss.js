...
function crearFormulario(){
  ...
  btn.addEventListener("click",function(){
    ...
    if(opt==="op1"){
      ...
      i1.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{f1=Number(i1.value);if(!Number.isInteger(f1)||f1<1||f1>n)throw 0;i2.focus();}catch(e){i1.value="";i1.focus();_err("Fila i no válida.");}});
      i2.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{let f2=Number(i2.value);if(!Number.isInteger(f2)||f2<1||f2>n||f2===f1)throw 0;
    ...
    if(opt==="op2"){
      ...
      i1.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{c1=Number(i1.value);if(!Number.isInteger(c1)||c1<1||c1>m)throw 0;i2.focus();}catch(e){i1.value="";i1.focus();_err("Columna i no válida.");}});
      i2.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{let c2=Number(i2.value);if(!Number.isInteger(c2)||c2<1||c2>m||c2===c1)throw 0;
    ...
    if(opt==="op4"){
      ...
      i1.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{a=Number(i1.value);if(!Number.isInteger(a)||a<1||a>n)throw 0;i2.focus();}catch(e){i1.value="";i1.focus();_err("Fila a no válida.");}});
      i2.addEventListener("keydown",function(ev){if(ev.key!=="Enter")return;try{let m=_parseRacional(i2.value);if(_esCeroNum(m))throw 0;
